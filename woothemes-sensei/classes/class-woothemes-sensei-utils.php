...
class WooThemes_Sensei_Utils {
	...
	public static function sensei_user_course_status_message( $course_id = 0, $user_id = 0 ) {
		...
		if( $course_id > 0 && $user_id > 0 ) {
			...
			if( $started_course ) {
				...
				$final_exam = end(Sensei()->course->course_lessons( $course_id ));
				$final_exam_status = WooThemes_Sensei_Utils::user_lesson_status($final_exam->ID);
				$user_grade = get_comment_meta($final_exam_status->comment_ID, 'grade', true);

				if( $user_grade >= $passmark ) {
					$status = 'passed';
					$box_class = 'tick';
					$message = sprintf( __( 'You have passed this course with a final exam grade of %1$d%%.', 'woothemes-sensei' ), $user_grade );
				} else {
					$status = 'failed';
					$box_class = 'alert';
	...
	public static function sensei_user_quiz_status_message( $lesson_id = 0, $user_id = 0, $is_lesson = false ) {
		global $woothemes_sensei, $current_user, $wpdb;
		if( intval( $user_id ) == 0 ) {
...
$term_list = wp_get_post_terms($lesson_id, 'lesson-tag', array("fields" => "all", "tag" => "final-exam"));
			if ( ! $started_course ) {
				...
				$message = sprintf( __( 'Please sign up for %1$sthe course%2$s before taking this exam', 'woothemes-sensei' ), '<a href="' . esc_url( get_permalink( $course_id ) ) . '" title="' . esc_attr( __( 'Sign Up', 'woothemes-sensei' ) ) . '">', '</a>' );
				}else{
				...
				$box_class = 'info';
				$message = __( 'You must be logged in to take this exam', 'woothemes-sensei' );
			...
			elseif ( $lesson_complete ) {
				...
				elseif ( ! empty( $quiz_grade ) && abs( $quiz_grade ) >= 0 ) {
                    ...
                    foreach ( $course_lessons as $lesson_item ) {
                            ...
                            $single_lesson_complete = WooThemes_Sensei_Utils::user_completed_lesson( $lesson_item->ID, $user_id );
                            if ( $single_lesson_complete ) {
                                $post_classes[] = 'lesson-completed';
                                $j++;
                            } // End If Statement
                    }
                    $progress_percentage = abs( round( ( doubleval( $j ) * 100 ) / ( $total_lessons ), 0 ) );
                    $table = $wpdb->prefix . "custom_table";
                    $user_count = $wpdb->get_var( "SELECT COUNT(*) FROM $table where user_id=".$user_id." && post_id=".$course_id."");
                    if($user_count>0){
                        $wpdb->update( $table,array( 'user_id' => $user_id, 'post_id' => $course_id , 'post_title' => $course_title,'complete_lesson' =>$j,'total_lesson' =>$total_lessons, 'course_percentage' =>$progress_percentage, 'last_seen' => $last_seen), array('post_id' => $course_id,'user_id' => $user_id));
                    }
                    else
                    {
                        $wpdb->insert( $table, array( 'user_id' => $user_id, 'post_id' => $course_id , 'post_title' => $course_title,'complete_lesson' =>$j,'total_lesson' =>$total_lessons, 'course_percentage' =>$progress_percentage, 'last_seen' => $last_seen));
                    }
					if( $is_lesson ) {
						if(count($term_list)>0){
						$message = sprintf( __( 'Congratulations! You have passed this lesson\'s exam achieving %s%%', 'woothemes-sensei' ), WooThemes_Sensei_Utils::round( $quiz_grade ) );
						}else{
						...
						if(count($term_list)>0){
						$message = sprintf( __( 'Congratulations! You have passed this exam achieving %s%%', 'woothemes-sensei' ),  WooThemes_Sensei_Utils::round( $quiz_grade ) );
					}else{
            ...
            // Lesson/Quiz not complete
				...
				if ( isset( $user_lesson_status->comment_approved ) && 'ungraded' == $user_lesson_status->comment_approved ) {
					...
					$box_class = 'info';
					if( $is_lesson ) {
						$message = sprintf( __( 'You have completed this lesson\'s exam and it will be graded soon. %1$sView the lesson exam%2$s', 'woothemes-sensei' ), '<a href="' . esc_url( get_permalink( $quiz_id ) ) . '" title="' . esc_attr( get_the_title( $quiz_id ) ) . '">', '</a>' );
					} else {
						$message = sprintf( __( 'You have completed this exam and it will be graded soon. You require %1$s%% to pass.', 'woothemes-sensei' ),  WooThemes_Sensei_Utils::round( $quiz_passmark ) );
					}
				}
				...
				elseif ( isset( $user_lesson_status->comment_approved ) && 'failed' == $user_lesson_status->comment_approved ) {
					...
					if( $is_lesson ) {
if(count($term_list)>0){
$message = sprintf( __( 'You are required to get a %1$d%% to pass the course exam. Your grade is %2$s%%', 'woothemes-sensei' ),  WooThemes_Sensei_Utils::round( $quiz_passmark ),  WooThemes_Sensei_Utils::round( $quiz_grade ) );
}
else{
$message = sprintf( __( 'You are required to get a %1$d%% to pass the lesson quiz. Your grade is %2$s%%', 'woothemes-sensei' ),  WooThemes_Sensei_Utils::round( $quiz_passmark ),  WooThemes_Sensei_Utils::round( $quiz_grade ) );
	}						
				
					} else {
						if(count($term_list)>0){
						$message = sprintf( __( 'You are required to get a  %1$d%% to pass this exam. Your grade is %2$s%%', 'woothemes-sensei' ),  WooThemes_Sensei_Utils::round( $quiz_passmark ),  WooThemes_Sensei_Utils::round( $quiz_grade ) );
						}else{
	$message = sprintf( __( 'You are required to get a  %1$d%% to pass this quiz. Your grade is %2$s%%', 'woothemes-sensei' ),  WooThemes_Sensei_Utils::round( $quiz_passmark ),  WooThemes_Sensei_Utils::round( $quiz_grade ) );						
							}
					}
				}
...
if(count($term_list)>0){
$message = sprintf( __( 'You are required to get a %1$d%% to pass the course exam.', 'woothemes-sensei' ),  WooThemes_Sensei_Utils::round( $quiz_passmark ) );
}
else{
$message = sprintf( __( 'You are required to get a %1$d%% to pass the lesson quiz.', 'woothemes-sensei' ),  WooThemes_Sensei_Utils::round( $quiz_passmark ) );
	}
						
					} else {
					if(count($term_list)>0){	
						$message = sprintf( __( 'You are required to get a %1$d%% to pass this exam.', 'woothemes-sensei' ),  WooThemes_Sensei_Utils::round( $quiz_passmark ) );
					}else{
						$message = sprintf( __( 'You are required to get a %1$d%% to pass this quiz.', 'woothemes-sensei' ),  WooThemes_Sensei_Utils::round( $quiz_passmark ) );
						}
					}
				}
			}
		...
		if( $is_lesson && ! in_array( $status, array( 'login_required', 'not_started_course' ) ) ) {
			...
			$extra = '<p><a class="button" href="' . esc_url( get_permalink( $quiz_id ) ) . '" title="' . esc_attr( apply_filters( 'sensei_view_lesson_quiz_text', __( 'Take Exam', 'woothemes-sensei' ) ) ) . '">' . apply_filters( 'sensei_view_lesson_quiz_text', __( 'Take Exam', 'woothemes-sensei' ) ) . '</a></p>';
				}
		}else{
$extra = '<p><a class="button" href="' . esc_url( get_permalink( $quiz_id ) ) . '" title="' . esc_attr( apply_filters( 'sensei_view_lesson_quiz_text', __( 'Take Exam', 'woothemes-sensei' ) ) ) . '">' . apply_filters( 'sensei_view_lesson_quiz_text', __( 'View or Take Quiz', 'woothemes-sensei' ) ) . '</a></p>';					
					}
		}
