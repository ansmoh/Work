...
class WooThemes_Sensei_Course
{
    ...
    public function __construct()
    {
        ...
        add_action('sensei_course_single_meta', array( $this, 'the_progress_statement' ), 16);
        add_action('sensei_course_single_meta', array( $this, 'the_progress_meter' ), 15);
         add_action('sensei_course_meter', array( $this, 'the_progress_statement' ), 16);
        add_action('sensei_course_meter', array( $this, 'course_progress_meter' ), 15);
    ...
    public function load_user_content($user = false, $manage = false)
    {
        global $woothemes_sensei, $post, $wp_query, $course, $my_courses_page, $my_courses_section, $total_user_course, $overall_progress, $wpdb;
        // Build Output HTML
        ...
        if ($user) {
            ...
            $state_exam = array_shift(get_posts(array(
                'title'    =>  'State Exam',
                'post_status'   =>  'publish',
                'post_type'     =>  'course'
            )));
            ...
            if ($state_exam){
                if (in_array($state_exam->ID, $completed_ids)){
                    foreach ($completed_ids as $key => $completed_id) {
                        if ($completed_id == $state_exam->ID)
                            unset($completed_ids[$key]);
                    }
                }

                if (in_array($state_exam->ID, $active_ids)){
                    foreach ($active_ids as $key => $active_id) {
                        if ($active_id == $state_exam->ID)
                            unset($active_ids[$key]);
                    }
                }
            }
            ...
            if (0 < intval(count($completed_ids))) {
             ...
             $total_user_course = $completed_count+$active_count;
             $overall_progress = abs(round(( doubleval(($completed_count/ $total_user_course)*100))));
            $lesson_count = 1;
            ...
            $active_html .= '<div class="course-box-1"><div class="box-1">
<div class="bg1"><div class="heading1">Courses Overview <a href="/my-courses/" class="vac">View All Courses</a></div>
<div class="course_title">Overall Progress<div class="meter"><span class="yellow" style="width:'.$overall_progress.'%">'.$overall_progress.'%</span></div>
<span class="complt">Completed '.$completed_count.' of '.$total_user_course.' courses
</div></div>
<div class="bg2"><div class="heading2">Current Course</div>';
            $table_name = $wpdb->prefix . "custom_table";
            $sql = "SELECT * FROM $table_name WHERE user_id=".$user->ID." ORDER BY last_seen DESC LIMIT 1";
            $results = $wpdb->get_results($sql);  // return an object, not ARRAY_N
            $i = $j = $k = $l =  0;
            if ($results) {
                foreach ($results as $row) {
                    $i = $row->total_lesson;
                     $j = $row->complete_lesson;
            // Each $row is a row from the query
                    $active_html .= '<div class="course_title">Course: <a href="'.get_permalink($row->post_id).'">'.$row->post_title . '</a></div><div class="meter"><span class="gray" style="width:'. $row->course_percentage .'%">'. $row->course_percentage .'%</span></div>';
                }
            }
            if ($results) {
                        $active_html .= '<a class="vcp" href="'.get_permalink($row->post_id).'">Visit Course Page</a>';
            } else {
                $active_html .='<div class="sensei-message info">You have not started any courses yet.</div>';
            }
            $active_html .= '</div></div></div><div class="course-box-2"><div class="box-1 box-4"><div class="bg1">'.do_shortcode('[gravityform id="8" name="Google Calander" title="false" description="false" ajax="true"]').'</div></div></div>';

            $active_html .= '<div class="clrsr"></div>';
            $active_html .= '<div class="course-box-3"><div class="box-1 box-2">
<div class="bg1"><div class="heading1">Lesson Overview</div><div class="title-part">';
            ...
            if ($results) {
                foreach ($results as $row) {
                    $k = $row->total_quiz_qus;
                    $l = $row->complete_quiz_qus;
                    $total_videos = $row->total_videos;
                    $total_ppt = $row->total_ppt;
                    if ($l == null) {
                        $l=0;
                    }
                      // Each $row is a row from the query <div class="current_course_title"><a href="'.get_permalink($row->course_id).'">'.$row->course_title.'</a></div>
                    $active_html .= '<div class="course_title">Current Lesson</div><a class="vcp" href="'.get_permalink($row->lesson_id).'">Continue Lesson</a><div class="current_course_title"><a href="'.get_permalink($row->lesson_id).'">'.$row->lesson_title . '</a></div>';
                }
            } else {
                    $active_html .='<div class="sensei-message info">You have not started any lesson yet.</div>';
            }

            $pbar = abs(round(intval($l*100)/$k));
            $active_html .= '</div></div><div class="bg2">
<li><div class="imgicon"><a href="'.get_permalink($row->lesson_id).'"><img src="/wp-content/uploads/2015/11/videos.png" /></a></div><div class="l-title">Videos:</div><div class="totals">Total 0</div></li>
<li><div class="imgicon"><a href="'.get_permalink($row->lesson_id).'"><img src="/wp-content/uploads/2015/11/pp.png" /></a></div><div class="l-title">Powerpoint Presentations:</div><div class="totals">Total '.$total_ppt.'</div></li>
<li><div class="imgicon"><a href="'.get_permalink($row->lesson_id).'"><img src="/wp-content/uploads/2015/11/books.png" /></a></div><div class="l-title">Books Chapters:</div><div class="totals">Read '.$j.' of '.$i.'</div></li>
<li><div class="imgicon"><a href="'.get_permalink($row->lesson_id).'"><img src="/wp-content/uploads/2015/11/quiz.png" /></a></div><div class="l-title">Quizzes:</div><div class="totals">Completed '.$l.' of '.$k.'</div></li>
<div class="p_bar"><span "course_title">Progress</span><div class="meter"><span class="gray" style="width:'. $pbar .'%">'. $pbar.'%</span></div></div>
</div></div></div>';
            $active_html .= '<div class="course-box-4"><div class="box-1 box-3">
<div class="bg1"><div class="heading1">Course Discussion & Members</div>
<div class="title-part"><div class="course_title"><a id="course-forum" class="activity-tabs active">Course Forum</a><a id="members" class="activity-tabs">Course Members</a></div>';
            $active_html .= '</div></div><div class="bg2"><a class="c-course" href="'.get_permalink($row->post_id).'">'.$row->post_title . '</a><div class="tab-contnt active" id="course-forum">'.do_shortcode('[bbp-topic-index]').'</div><div class="tab-contnt" id="members">'.do_shortcode('[bc_members amount="2"]').'</div>
</div></div></div>';
...
<?php /*?>
    ...
    public function course_lessons_meta_box_content()
    {
        ...
        if (0 < $post->ID) {
            $select_course_prerequisite = get_post_meta($post->ID, '_lesson_course', true);
        }
        ...
        if (count($posts_array) > 0) {
            $html .='<ul id="sortable" class="ui-sortable">';
            foreach ($posts_array as $post_item) {
                $html .= '<p class="ui-state-default">'."\n";
...
//					}
    ...
    public function courses_certificates($user = false, $manage = false)
    {
        global $woothemes_sensei, $post, $wp_query, $course, $my_courses_page, $my_courses_section, $total_user_course, $overall_progress, $wpdb;
        ...
        if ($user) {
            ...
            foreach ($completed_courses as $course_item) {
                ...
                $complete_html .= '<section class="entry">';
                    ...
                    // Title
                    $complete_html .= '<header class="ctitle">';
                ...
                if ($manage) {
                    ...
                    if (has_filter('sensei_results_links') || $has_quizzes) {
                        $complete_html .= '<p class="sensei-results-links certibuttons">';
                        $results_link = '';
                        if ($has_quizzes) {
                            $results_link = '<a class="button view-results" target="_blank" href="' . $woothemes_sensei->course_results->get_permalink($course_item->ID) . '">' . apply_filters('sensei_view_results_text', __('View results', 'woothemes-sensei')) . '</a>';
                        }
                    ...
                    $complete_html .= '<br><span class="bdr"><img src="/wp-content/uploads/2016/01/bdr.jpg"></span></section>';
    ...
    public function all_courses_content($user = false, $manage = false)
    {
        global $woothemes_sensei, $post, $wp_query, $course, $my_courses_page, $my_courses_section, $total_user_course, $overall_progress, $wpdb;
        ...
        /*** WisdmLabs Changes start ***/
            $active_courses= apply_filters('wdm_change_order_of_active_courses', $active_courses);
            ...
            foreach ($active_courses as $course_item) {
                ...
                $active_html .= '<section class="entry halfcol">';
      // Image
                ...
                if ('' != $category_output) {
                    ...
                    $active_html .= '<span class="course-lesson-progress">' . sprintf(__('%1$d of %2$d <a>lessons completed</a>', 'woothemes-sensei'), $lessons_completed, $lesson_count) . '</span>';

                    $active_html .= '</p>';

                    $active_html .= '<p class="course-excerpt">' . apply_filters('get_the_excerpt', $course_item->post_excerpt) . '<br><span class="bdr"><img src="/wp-content/uploads/2016/01/bdr.jpg"></span></p>';
                ...
                if ($manage) {
                    $active_html .= '<section class="entry-actions halfcol">';
                    $active_html .= '<span class="p-title">Overall Progress</span><div class="courselink"><a href="' . esc_url(get_permalink(absint($course_item->ID))) . '" title="' . esc_attr($course_item->post_title) . '">Open This Course</a></div>';
                    $progress_percentage = abs(round(( doubleval($lessons_completed) * 100 ) / ( $lesson_count ), 0));

                    $active_html .= $this->get_progress_meter($progress_percentage);
                    $active_html .= '<span class="course-lesson-progress">Completed ' . sprintf(__('%1$d of %2$d lessons', 'woothemes-sensei'), $lessons_completed, $lesson_count) . '</span>';
    ...
    */
            ...
            foreach ($completed_courses as $course_item) {
                ...
                //	$complete_html .= '<span class="course-category">' . sprintf( __( 'in %s', 'woothemes-sensei' ), $category_output ) . '</span>';
                        ...
                        $complete_html .= '<p class="course-excerpt">' . apply_filters('get_the_excerpt', $course_item->post_excerpt) . '<br><span class="bdr"><img src="/wp-content/uploads/2016/01/bdr.jpg"></span></p>';
        ...
        if ($manage) {
            ...
            <ul class="course-activity">
                <li><a href="#active-courses"><?php echo apply_filters('sensei_active_courses_text', __('Active Courses', 'woothemes-sensei')); ?></a></li>
                <li><a href="#completed-courses"><?php echo apply_filters('sensei_completed_courses_text', __('Completed Courses', 'woothemes-sensei')); ?></a></li>
                <a class="purchase" href="https://carealtyschool.com/purchase/"><strong>Purchase More Courses</strong></a>
                <?php
                /*** WisdmLabs Changes Start ***/
                do_action('wdm_sensei_add_more_options');
                /*** WisdmLabs Changes End ***/
                ?>
            </ul>
            ...
            if (0 < $course_page_id) {
                ...
                $course_page_url = get_post_type_archive_link('course');
                    ...
                    <div id="active-courses">
                    <h2 class="courseactivity">Active Courses</h2>
                    <?php if ('' != $active_html) {
                        echo $active_html;
} else { ?>
                        <div class="sensei-message info"><?php echo $no_active_message; ?> <a href="/purchase"><?php apply_filters('sensei_start_a_course_text', _e('Start a Course!', 'woothemes-sensei')); ?></a></div>
                    <?php                                             } // End If Statement ?>
                    ...
                    <div id="completed-courses">
        <h2 class="courseactivity">Completed Courses</h2>
                        <?php if ('' != $complete_html) {
                            echo $complete_html;
} else { ?>
                        <div class="sensei-message info"><?php echo $no_complete_message; ?></div>
    ...
    public function load_user_courses_content($user = false, $manage = false)
    {
        ...
        if ($user) {
            ...
            foreach ($active_courses as $course_item) {
                ...
                $active_html .= '<article id="dashpost" class="' . esc_attr(join(' ', get_post_class(array( 'course', 'post' ), $course_item->ID))) . '">';
...
*/
    ...
    public function get_progress_meter($progress_percentage)
    {
        ...
        $progress_bar_html = '<div class="meter"><span class="' . esc_attr($class) . '" style="width: ' . $progress_percentage . '%">' . round($progress_percentage) . '%</span></div>';
     ...
     * @return void
     */
    public function course_progress_meter($course_id = 0, $user_id = 0)
    {
        ...
        $percentage_completed = $this->get_completion_percentage($course_id, $user_id);
        echo '<div class="course_title">Overall Progress</div>';
        echo $this->get_progress_meter($percentage_completed);
