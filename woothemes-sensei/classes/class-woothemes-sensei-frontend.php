...
class WooThemes_Sensei_Frontend
{
    ...
    public function enqueue_styles()
    {
        ...
        if (! $disable_styles) {
            wp_register_style($woothemes_sensei->token . '-frontend', $woothemes_sensei->plugin_url . 'assets/css/frontend.css', '', time(), 'screen');
    ...
    function sensei_locate_template($template_name, $template_path = '', $default_path = '')
    {
        ...
        if (! $template_path) {
            $template_path = $woothemes_sensei->template_url;
        }
        if (! $default_path) {
            $default_path = $woothemes_sensei->plugin_path() . 'templates/';
        }
    ...
    function sensei_single_title()
    {
        ...
        if (is_singular('sensei_message')) {
            ...
            if (count($term_list)>0) {
                        $title = get_the_title() . ' ' . __('', 'woothemes-sensei');
            } else {
...
if (WooThemes_Sensei_Utils::sensei_is_woocommerce_activated() && ( 0 < $wc_post_id )) {
    ...
    public function sensei_complete_lesson_button()
    {
        ...
        if (! $quiz_id || ( $quiz_id && ! $pass_required )) {
            ...
            <form class="lesson_button_form" method="POST" action="<?php echo esc_url(get_permalink()); ?>#lesson_complete">
                ...
                <?php /*?> <span><input type="submit" name="quiz_complete" class="quiz-submit complete" value="<?php echo apply_filters( 'sensei_complete_lesson_text', __( 'Complete Lesson', 'woothemes-sensei' ) ); ?>"/></span><?php */?>
            </form>
            <?php
        } // End If Statement
    ...
    public function sensei_reset_lesson_button()
    {
        ...
        $quiz_id_pass = $woothemes_sensei->post_types->lesson->lesson_quizzes($lesson_id);
        // Quiz passmark
        ...
        <form method="POST" action="<?php echo esc_url(get_permalink()); ?>">
            ...
            <?php
            if (count($term_list)>0) {
                if ($user_quiz_grade < $quiz_passmark) {
                                ?>
    ...
    public function sensei_lesson_quiz_meta($post_id = 0, $user_id = 0)
    {
        ...
        if (intval($lesson_prerequisite) > 0) {
            ...
            if (current_user_can('administrator')) {
                echo '<div class="sensei-message alert transbg">This Lesson has prerequisite set up by adminstrator.</div>';
            }
        }
        ?><header><?php
...
if ($quiz_id && is_user_logged_in() && WooThemes_Sensei_Utils::user_started_course($lesson_course_id, $user_id)) { ?>
        ...
        <?php         } elseif ($show_actions && $quiz_id && $woothemes_sensei->access_settings()) { ?>
            <?php
                $has_quiz_questions = get_post_meta($post_id, '_quiz_has_questions', true);
            if ($has_quiz_questions) { ?>
                    <p><a class="button" href="<?php echo esc_url(get_permalink($quiz_id)); ?>" title="<?php echo esc_attr(apply_filters('sensei_view_lesson_quiz_text', __('View the Lesson Exam', 'woothemes-sensei'))); ?>"><?php echo apply_filters('sensei_view_lesson_quiz_text', __('View the Lesson Exam', 'woothemes-sensei')); ?></a></p>
                <?php                                                                           } ?>
        <?php }
if ($show_actions && ! $has_user_completed_lesson) {
    sensei_complete_lesson_button();
}
 //Wisdmlabs commented the code for remove reset lesson button
//elseif ($show_actions) {
...
//} // End If Statement
    ...
    public function sensei_quiz_action_buttons()
    {
        ...
        $quiz_id_pass = $woothemes_sensei->post_types->lesson->lesson_quizzes($lesson_id);
        // Quiz passmark
            $quiz_passmark = absint(get_post_meta($quiz_id_pass, '_quiz_passmark', true));
            $quiz_passmark_float = (float) $quiz_passmark;
            $term_list = wp_get_post_terms($lesson_id, 'lesson-tag', array("fields" => "all", "tag" => "final-exam"));
        //setup quiz grade
        $user_quiz_grade = '';
            ...
            if ('' == $user_quiz_grade) {
                if (count($term_list)>0) {
                    ?>
                            ...
                            <span><input type="submit" id="cexam" name="quiz_complete" class="quiz-submit complete" value="<?php echo apply_filters('sensei_complete_quiz_text', __('Complete Exam', 'woothemes-sensei')); ?>"/></span>
                            <span><input type="submit" name="quiz_save" class="quiz-submit save" value="<?php echo apply_filters('sensei_save_quiz_text', __('Save Exam Answers', 'woothemes-sensei')); ?>"/></span>
                <?php
                } else { // End If Statement
                ?>

  <span><input type="submit" id="cquiz" name="quiz_complete" class="quiz-submit complete" value="<?php echo apply_filters('sensei_complete_quiz_text', __('Complete Quiz', 'woothemes-sensei')); ?>"/></span>
...
if ($user_quiz_grade < $quiz_passmark) {
    if (isset($reset_quiz_allowed) && $reset_quiz_allowed) {
        if (count($term_list)>0) { ?>
                           <span><input type="submit" name="quiz_reset" class="quiz-submit reset" value="<?php echo apply_filters('sensei_reset_quiz_text', __('Reset Exam', 'woothemes-sensei')); ?>"/></span>
                            <?php                                                                                                                                                                                                                                                                                                                                                                                                                         } else {
                    ?>
