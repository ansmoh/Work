...
class WooThemes_Sensei_Messages {
	...
	public function __construct () {
        ...
        // Add contact lesson tutor button
		add_action( 'sensei_lesson_tutor', array($this,'send_message_link' ));
		...
		if( ! isset( $woothemes_sensei->settings->settings['messages_disable'] ) || ! $woothemes_sensei->settings->settings['messages_disable'] ) {
			...
			if( isset( $_GET['contact'] ) ) {
				...
				if( 'lesson' == $post->post_type ) {
					$contact_button_text = __( 'Contact Lesson Tutor', 'woothemes-sensei' );
				} elseif( 'course' == $post->post_type ) {
					$contact_button_text = __( 'Contact Course Trainer', 'woothemes-sensei' );
				}else{
                    $contact_button_text = __( 'Contact Lesson Tutor', 'woothemes-sensei' );
                }

				$html .= '<span class="bp-sensei-msg-link"><a class="button send-message-button" href="'. esc_url($href) . '#private_message">' . $contact_button_text . '</a></span>';
			}
	...
	public function teacher_contact_form( $post ) {
        ...
        if( isset( $_GET[ 'send' ] ) && 'complete' == $_GET[ 'send' ] ) {
            ...
            $confirmation_message = __('Please allow 1-2 days for a response for a Trainer. Thanks', 'woothemes-sensei');
			$confirmation .= '<script language="javascript">';
$confirmation .= 'window.location.href = window.location.href.split("?")[0];';			
$confirmation .= 'alert("Please allow 1-2 days for a response for a Trainer. Thanks!")';
$confirmation .= '</script>';
		...
		$html .= '<form name="contact-teacher" action="" method="post" class="contact-teacher">';
			$html .= '<p class="form-row form-row-wide">';
				$html .= '<textarea name="contact_message" placeholder="' . __( 'Enter your message.', 'woothemes-sensei' ) . '"></textarea>';
			$html .= '</p>';
			$html .= '<p class="form-row">';
				// $user = get_user_by('email', 'trainer@carealtytraining.com');
		...
		return '<div class="private-msg">'.$html.'</div>';
	}
    ...
    $current_user = wp_get_current_user();
		...
		wp_mail( 'trainer@carealtytraining.com', 'Email from '.$current_user->user_email, $_POST['contact_message'] );
    ...
    private function save_new_message_post( $sender_id = 0, $receiver_id = 0, $message = '', $post_id = 0 ) {
    	...
    	if( $sender_id && $receiver_id && $message && $post_id ) {
	    	...
	    	wp_mail( 'trainer@carealtytraining.com', $title, $messages );
