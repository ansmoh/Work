...
class WooThemes_Sensei_Learner_Profiles {
	...
	global $wpdb,$current_user;
     ...
     $learner_avatar = $_SESSION['learner_avatar'] = apply_filters( 'sensei_learner_profile_info_avatar', get_avatar( $current_user->ID, 120 ), $current_user->ID );
		$learner_name = $_SESSION['leaner_name'] = apply_filters( 'sensei_learner_profile_info_name', $user->display_name, $current_user->ID );
		$learner_bio = $_SESSION['learner_bio'] = apply_filters( 'sensei_learner_profile_info_bio', $user->description, $current_user->ID );
			?>
		<div id="learner-info">
			<div class="learner-avatar"><form role="search" class="searchform" method="get" action="<?php echo home_url( '/' ); ?>">
	<?php /*?><div class="search-table">
...
</form><div class="avatar"><div class="profilenav"><a href="/members/<?php echo $current_user->user_nicename; ?>"><span class="helloword">Hello,</span> <?php echo array_shift(explode(' ', $current_user->display_name));?></a><a href="<?php echo wp_logout_url( home_url() ); ?>" title="Logout">Logout</a></div><a href="/members/<?php echo $current_user->user_nicename; ?>/profile/change-avatar/"><?php echo $learner_avatar; ?></a></div></div>
			<div class="learner-content">
				<div class="description"><span class="welcome-txt">Welcome <?php echo $current_user->first_name .' '. $current_user->last_name;?>,</span><br>
CA Realty School Portal</div>
			</div>
<div class="nav-map"><i class="fa fa-home"></i>
<?php if (function_exists('qt_custom_breadcrumbs')) qt_custom_breadcrumbs(); ?></div>            
		</div>
