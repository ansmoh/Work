...
$sql_quiz_qus = $wpdb->get_results("SELECT complete_quiz_qus FROM $table_name WHERE lesson_id=".$lesson_id."");
if(count($sql_quiz_qus)>0){
$wpdb->update( $table_name,array('complete_quiz_qus' => count($prepared_answers)), array('lesson_id' => $lesson_id, 'user_id' => $user_id)); 
	}
else
{
$wpdb->insert( $table_name, array('complete_quiz_qus' => count($prepared_answers)),array('lesson_id' => $lesson_id, 'user_id' => $user_id));
	}	
		if( count($prepared_answers) > 0){
            ...
            set_transient( $transient_key, $prepared_answers, 10 * DAY_IN_SECONDS );
		            // update the message showed to user
			            $woothemes_sensei->frontend->messages = '<div class="sensei-message note">' . apply_filters( 'sensei_quiz_saved_text', __( 'Exam Saved Successfully.', 'woothemes-sensei' ) ) . '</div>';
        }
...
$sql_quiz_qus = $wpdb->get_results("SELECT complete_quiz_qus FROM $table_name WHERE lesson_id=".$lesson_id."");	
if(count($sql_quiz_qus)>= 0){
$wpdb->update( $table_name,array('complete_quiz_qus' => 0), array('lesson_id' => $lesson_id ,'user_id' => $user_id)); 
	}
        ...
        $woothemes_sensei->frontend->messages = '<div class="sensei-message note">' . apply_filters( 'sensei_quiz_reset_text', __( 'Exam Reset Successfully.', 'woothemes-sensei' ) ) . '</div>';
