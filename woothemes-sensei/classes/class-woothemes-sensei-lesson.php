    ...
    public function lesson_prerequisite_meta_box_content()
    {
        ...
        if (count($posts_array) > 0) {
            ...
            foreach ($posts_array as $post_item) {
                $html .= '<option alt="'.get_permalink($post_item->ID).'" value="' . esc_attr(absint($post_item->ID)) . '"' . selected($post_item->ID, $select_lesson_prerequisite, false) . '>' . esc_html($post_item->post_title) . '</option>' . "\n";
            } // End For Loop
            $html .= '</select>' . "\n";
        } else {
