...
class WC_Shipstation_API_Shipnotify extends WC_Shipstation_API_Request {
	...
	public function request() {
		...
		if ( sizeof( $shipped_items ) > 0 ) {
			$order_note            = sprintf( __( '%s shipped via %s on %s with tracking number <b>%s</b>.', 'woocommerce' ), esc_html( implode( ', ', $shipped_items ) ), esc_html( $carrier ), date_i18n( get_option( 'date_format' ), $timestamp ), $tracking_number );
		...
		// If we don't have items, just complete the order as a whole
			...
			$order_shipped = true;
			$order_note    = sprintf( __( 'Items shipped via %s on %s with tracking number <b>%s</b>.', 'woocommerce' ), esc_html( $carrier ), date_i18n( get_option( 'date_format' ), $timestamp ), $tracking_number );
