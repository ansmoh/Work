...
function sensei_update_users_certificate_data( $n = 5, $offset = 0 ) {
	...
	if ( 0 == $offset ) {
		...
		$current_page = intval( $offset / $n ) -1;
	} // End If Statement
//var_dump('$offset',$offset,'$n', $n,'$current_page', $current_page);
	$args_array = array(
			'number' => $n,
			'offset' => $offset,
			'orderby' => 'ID',
...
//		$course_ids = WooThemes_Sensei_Utils::sensei_activity_ids( array( 'user_id' => $user_item->ID, 'type' => 'sensei_course_start' ) );
    $course_statuses = WooThemes_Sensei_Utils::sensei_check_for_activity(array( 'user_id' => $user_item->ID, 'type' => 'sensei_course_status' ), true);
    ...
    foreach ($course_statuses as $course_status) {
        if (WooThemes_Sensei_Utils::user_completed_course($course_status, $user_item->ID)) {
            $completed_ids[] = $course_status->comment_post_ID;
        } else {
            $active_ids[] = $course_status->comment_post_ID;
        }
    }
//    var_dump($user_item->ID, '<hr/>');
//    if ($user_item->ID == 223) {
//      var_dump('<hr/>', '<hr/>', '<hr/>', '<hr/>', '<hr/>', '<hr/>', '<hr/>', $completed_ids);die;
//    }
//		$posts_array = array();
...
//		foreach ( $posts_array as $course_item ) {
    foreach ( $completed_ids as $course_item ) {  
...
//			if ( isset( $course_end_date ) && '' != $course_end_date ) {
				...
				if ( ! $query->have_posts() ) {
					...
					$cert_args = array(
						...
						'post_title' => esc_html( substr( md5( $course_item . $user_item->ID ), -8 ) ),
						'post_name' => esc_html( substr( md5( $course_item . $user_item->ID ), -8 ) ),
						'post_type' => 'certificate',
						'post_status'   => 'publish'
					);
					...
					if ( ! is_wp_error( $post_id ) ) {
						add_post_meta( $post_id, 'course_id', intval( $course_item ) );
						add_post_meta( $post_id, 'learner_id', intval( $user_item->ID ) );
						add_post_meta( $post_id, 'certificate_hash',esc_html( substr( md5( $course_item . $user_item->ID ), -8 ) ) );
						$loop_ran = true;
					} // End If Statement
