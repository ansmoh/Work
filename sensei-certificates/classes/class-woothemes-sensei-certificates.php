...
class WooThemes_Sensei_Certificates
{
    ...
    public function can_view_certificate($certificate_id = 0)
    {
        ...
        if (true == (bool)$grant_access_user || ( false == (bool)$grant_access && true == (bool)$grant_access_user ) || current_user_can('manage_options')) {
            return true;
        }
    ...
    public function certificate_text($pdf_certificate, $fpdf)
    {
        ...
        if (0 < intval($certificate_id)) {
            ...
            //convert UTC time to local wordpress time.
            $registered = get_date_from_gmt($registered, $date_format);
            ...
            if (isset($this->certificate_template_fields['certificate_heading']['text']) && '' != $this->certificate_template_fields['certificate_heading']['text']) {
                ...
                $certificate_heading = str_replace(array( '{{learner}}', '{{course_title}}', '{{completion_date}}', '{{course_place}}','{{registration_date}}'  ), array( $student_name, $course->post_title, $date, get_bloginfo('name'),$registered ), $certificate_heading);
            } // End If Statement
            ...
            if (isset($this->certificate_template_fields['certificate_message']['text']) && '' != $this->certificate_template_fields['certificate_message']['text']) {
                ...
                $certificate_message = str_replace(array( '{{learner}}', '{{course_title}}', '{{completion_date}}', '{{course_place}}','{{registration_date}}'  ), array( $student_name, $course->post_title, $date, get_bloginfo('name'),$registered ), $certificate_message);
            } // End If Statement
            ...
            if (isset($this->certificate_template_fields['certificate_course']['text']) && '' != $this->certificate_template_fields['certificate_course']['text']) {
                ...
                $certificate_course = str_replace(array( '{{learner}}', '{{course_title}}', '{{completion_date}}', '{{course_place}}','{{registration_date}}'  ), array( $student_name, $course->post_title, $date, get_bloginfo('name') ,$registered), $certificate_course);
            } // End If Statement
            ...
            if (isset($this->certificate_template_fields['certificate_completion']['text']) && '' != $this->certificate_template_fields['certificate_completion']['text']) {
                ...
                $certificate_completion = str_replace(array( '{{learner}}', '{{course_title}}', '{{completion_date}}', '{{course_place}}','{{registration_date}}'  ), array( $student_name, $course->post_title, $date, get_bloginfo('name'),$registered ), $certificate_completion);
            } // End If Statement
            ...
            if (isset($this->certificate_template_fields['certificate_place']['text']) && '' != $this->certificate_template_fields['certificate_place']['text']) {
                ...
                $certificate_place = str_replace(array( '{{learner}}', '{{course_title}}', '{{completion_date}}', '{{course_place}}' ,'{{registration_date}}' ), array( $student_name, $course->post_title, $date, get_bloginfo('name'),$registered  ), $certificate_place);
            } // End If Statement
...
/** WisdmLabs changes start **/
            ...
            $certificate_registration_date = $registered; // At {{course_place}}
            if (isset($this->certificate_template_fields['certificate_registration_date']['text']) && '' != $this->certificate_template_fields['certificate_registration_date']['text']) {
                $certificate_place = $this->certificate_template_fields['certificate_registration_date']['text'];
                $certificate_place = str_replace(array( '{{learner}}', '{{course_title}}', '{{completion_date}}', '{{course_place}}' ,'{{registration_date}}' ), array( $student_name, $course->post_title, $date, get_bloginfo('name'),$registered  ), $certificate_registration_date);
            } // End If Statement
 ...
 /** WisdmLabs changes end **/
            ...
            $output_fields = array(     'certificate_heading'       => 'text_field',
                                    ...
                                    'certificate_registration_date' => 'text_field',
                                 );
    ...
    public function certificate_link($message)
    {
        ...
        if (( is_page($my_account_page_id) || is_singular('course') || is_page('certificates') || isset($wp_query->query_vars['course_results']) ) && $view_link_courses) {
            $is_viewable = true;
        ...
        if ($is_viewable) {
            ...
            if ('' != $certificate_url) {
                ...
                $message = $message . '<a href="' . $certificate_url . '" class="' . $classes . 'sensei-certificate-link" target="_blank" title="' . esc_attr(__('View Certificate', 'sensei-certificates')) . '">'. __('View Certificate', 'sensei-certificates') . '</a>';
            } // End If Statement
        } // End If Statement
    ...
    public function populate_columns($content, $item, $analysis)
    {
        ...
        if ('user' == $analysis->view) {
            ...
            if ('' != $certificate_url) {
                $output = '<a href="' . $certificate_url . '" class="sensei-certificate-link" target="_blank" title="' . esc_attr(__('View Certificate', 'sensei-certificates')) . '">'. __('View Certificate', 'sensei-certificates') . '</a>';
            } // End If Statement
