<?php
/**
 * Plugin Name: WooCommerce Shop as Customer (shared on wplocker.com)
 ...
 * Define Constants
 **/
define( 'SHOP_AS_CUSTOMER_VERSION', '1.08' );
define( 'SHOP_AS_CUSTOMER_REQUIRED_WOOCOMMERCE_VERSION', 2.2 );

/**
 * Update Check
 */
require 'plugin-updates/plugin-update-checker.php';
require 'plugin-updates/woocp.php';
new PluginUpdateChecker(
	'http://cxthemes.com/plugin/woocommerce-shop-as-customer/shop-as-customers.json',
	__FILE__,
	'shop-as-customer'
);
 ...
 */
if ( ! is_woocommerce_active() || version_compare( get_option( 'woocommerce_db_version' ), SHOP_AS_CUSTOMER_REQUIRED_WOOCOMMERCE_VERSION, '<' ) ) {
	add_action( 'admin_notices', 'WC_Shop_As_Customer::woocommerce_inactive_notice' );
	return;
}
 ...
 *
 */
$GLOBALS['wc_shop_as_customer'] = new WC_Shop_As_Customer();
...
class WC_Shop_As_Customer {
	...
	public function load_translation() {
		...
		// localisation
		load_plugin_textdomain( 'shop-as-customer', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
	...
	public function admin_scripts() {
    	...
    	/* Top Drop-Down Stuff */
		wp_register_style( 'woocommerce-shop-as-customer', plugins_url( basename( plugin_dir_path( __FILE__ ) ) . '/css/shop-as-customer-styles.css', basename( __FILE__ ) ), '', SHOP_AS_CUSTOMER_VERSION, 'screen' );
		...
		/* Options Page Stuff */
		wp_register_style( 'woocommerce-shop-as-customer-options', plugins_url( basename( plugin_dir_path( __FILE__ ) ) . '/css/options-page-style.css', basename( __FILE__ ) ), '', SHOP_AS_CUSTOMER_VERSION, 'screen' );
	...
	public function redirect_customer_on_order_processed($order_id, $posted = null) {
		...
		if ( ( isset( $_POST['woocommerce_checkout_save_order'] ) ) || ( ! isset( $_POST["payment_method"] ) ) ) {
			...
			if ( version_compare( $woocommerce->version, '2.1', '<' ) ) {
				...
				$thanks_page = add_query_arg( 'key', $order->order_key, add_query_arg( 'order', $order_id, add_query_arg( 'order_on_behalf', 1, $thanks_page ) ) );
			} else {
				$thanks_page = wc_get_endpoint_url( 'order-received', $order_id, get_permalink( wc_get_page_id( 'checkout' ) ) );
				$thanks_page = add_query_arg( 'key', $order->order_key, add_query_arg( 'order_on_behalf', 1, $thanks_page ) );
			}
	...
	public function post_checkout_as_customer_options($order_id) {
		...
		$to_order_link = add_query_arg( array(
		...
		switch ( $_REQUEST['action'] ) {
			...
			case 'shop_as_customer':
				...
				if ( shop_as_customer( $user_id, self::remember() ) ) {
					...
					if ( $redirect_to )
						wp_safe_redirect( add_query_arg( array( 'shopping_as_customer' => 'true' ), $redirect_to ) );
					else if ( !current_user_can( 'read' ) )
						wp_redirect( add_query_arg( array( 'shopping_as_customer' => 'true' ), get_permalink( woocommerce_get_page_id('myaccount') ) ) );
					else
						wp_redirect( add_query_arg( array( 'shopping_as_customer' => 'true' ), get_permalink( woocommerce_get_page_id('myaccount') ) ) );
					die();
			...
			case 'back_to_old_user':
				...
				if ( shop_as_customer( $old_user->ID, self::remember(), false ) ) {
					if ( isset( $_REQUEST['redirect_to_order'] ) and !empty( $_REQUEST['redirect_to_order'] ) ) {
						...
						if ( $redirect_to )
							wp_safe_redirect( add_query_arg( array( 'shopping_as_customer' => 'true', 'switched_back_user' => 'true' ), $redirect_to ) );
						else
							wp_redirect( add_query_arg( array( 'shopping_as_customer' => 'true', 'switched_back_user' => 'true' ), admin_url( 'users.php' ) ) );
					}
					die();
				} else {
					wp_die( __( 'Could not switch back to originating user.', 'shop-as-customer' ) );
	...
	public function build_default_originating_user_menu() {
		...
		if ( $all_switched_users ) {
			...
			foreach ( $all_switched_users as $swited_user ) {
				...
				$user_link = esc_url( network_admin_url( 'user-edit.php?user_id=' . $swited_user->ID ) );
			...
			if ( ! is_woocommerce_active() ) :
				...
				<div id="message" class="error">
					<p>
						<?php
						printf(
							__( '%sShop as Customer for WooCommerce is inactive%s %sWooCommerce%s must be active for Shop as Customer to work. Please install & activate WooCommerce.', 'shop-as-customer' ),
							'<strong>',
							'</strong><br>',
				...
				<?php
			elseif ( version_compare( get_option( 'woocommerce_db_version' ), SHOP_AS_CUSTOMER_REQUIRED_WOOCOMMERCE_VERSION, '<' ) ) :
				?>
				<div id="message" class="error">
					...
					<p>
						...
						printf(
							...
							'<strong>',
							'</strong><br>',
							SHOP_AS_CUSTOMER_REQUIRED_WOOCOMMERCE_VERSION,
